...
└── frontend/
    ...
    └── src/
        ...
        │   │   └── ExpenseForm.js
        │   ├── Form/
        │   │   └── Form.js       # Reusable form component (used for Income)
        │   ├── History/
        │   │   └── History.js
        │   ├── Income/
        │   │   └── Income.js
        │   ├── IncomeItems/
        │   │   └── IncomeItems.js # Component to display single transaction item
        │   ├── Navigation/
        │   │   └── Navigation.js
        │   ├── Orb/
        │   │   └── Orb.js
        │   └── View/
        │       └── View.js       # Component for viewing all transactions
        ├── context/
...
Data Flow
Authentication: A new user registers or an existing user logs in. The backend validates the credentials, hashes the password using bcrypt, and returns a JWT.

Session Management: The JWT is stored in the client's local storage and sent in the Authorization header with every subsequent API request to authenticate the user.

Transaction Management: A logged-in user adds a new income or expense. The data, linked to their user ID, is sent via a protected API route and saved to the MongoDB database.

Data Fetching: The frontend fetches the user's income and expense data from the backend. This data is then used to populate the dashboard, charts, and transaction lists.

Data Export: A user requests to download their data. The backend fetches all their transactions, generates an Excel file using ExcelJS, and sends it back to the client for download.
...
Usage
Register/Login: Open the application and create a new account or log in.

View Dashboard: After logging in, you will be taken to the main dashboard with a summary of your finances and a chart.

Add Income/Expense: Use the "Incomes" or "Expenses" tabs to add new transactions via the provided forms.

View Transactions: Navigate to "View Transactions" to see a complete history of your financial activity.

Download Report: From the dashboard, click the download button to get an Excel file of all your data.
